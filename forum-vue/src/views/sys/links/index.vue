<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tableData" ref="tableRef" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="链接" align="center" prop="links" min-width="120" :show-overflow-tooltip='true'/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="700px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
        ...
        <el-form-item label="链接" prop="links">
          <el-input v-model="form.links" placeholder="请输入链接" clearable/>
        </el-form-item>
      </el-form>
...
import { dataLinks, dynamicListLinks, listLimitLinks, countLinks, dynamicCountLinks, allLinks, getLinks, detailsGetLinks, dynamicGetLinks, addLinks, editLinks, delLinks, delListLinks } from "@/api/links";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        links: [{ required: true, trigger: 'blur', validator: validateData }],
      }
    };
  },
  created() {
  ...
  methods: {
    ...
    getData() {
      this.loading = true;
      dataLinks(this.queryParams).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            editLinks(this.form).then(response => {
    ...
    handleExport() {
       import('@/vendor/Export2Excel').then(excel => {
           // 表格名称
           const columnNames = ['友情链接ID','名称','链接'];
           // 表格属性
           const headerProps = ['id','name','links'];
           // 表格数据
