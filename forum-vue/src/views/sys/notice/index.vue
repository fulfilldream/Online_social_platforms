...
import { dataNotice, dynamicListNotice, listLimitNotice, countNotice, dynamicCountNotice, allNotice, getNotice, detailsGetNotice, dynamicGetNotice, addNotice, editNotice, delNotice, delListNotice } from "@/api/notice";
...
export default {
  ...
  methods: {
    ...
    /** 新增按钮操作 */
    handleAdd() {
        this.$router.push({ path: '/sys-basics/notice/add'})
    },
    /** 编辑按钮操作 */
    handleUpdate(row) {
        this.$router.push({ path: '/sys-basics/notice/edit/' + row.id })
    },
    /** 详情按钮操作 */
    handleDetails(row) {
        this.$router.push({ path: '/sys-basics/notice/details/' + row.id })
    },
    // 单个删除按钮操作
    handleDelete(row) {
