<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tableData" ref="tableRef" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="交流小组" align="center" prop="groups.name" min-width="120" :show-overflow-tooltip='true'/>
      <el-table-column label="交流用户" align="center" prop="user.name" min-width="120" :show-overflow-tooltip='true'/>
      <el-table-column label="交流内容" align="center" prop="content" min-width="120" :show-overflow-tooltip='true'/>
      <el-table-column label="交流时间" align="center" prop="creatTime" min-width="120" :show-overflow-tooltip='true'/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="700px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
        ...
        <el-form-item label="类型" prop="types">
          <el-input v-model.number="form.types" placeholder="请输入类型" clearable/>
        </el-form-item>
      </el-form>
...
import { dataGroupsComment, dynamicListGroupsComment, listLimitGroupsComment, countGroupsComment, dynamicCountGroupsComment, allGroupsComment, getGroupsComment, detailsGetGroupsComment, dynamicGetGroupsComment, addGroupsComment, editGroupsComment, delGroupsComment, delListGroupsComment } from "@/api/groupsComment";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        types: [{ required: true, trigger: 'blur', validator: validateNumberData }],
      }
    };
  },
  created() {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getGroupsComment(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            editGroupsComment(this.form).then(response => {
            ...
            addGroupsComment(this.form).then(response => {
    ...
    handleDelete(row) {
       this.$confirm('确定删除吗?', '提示', {
           ...
           return delGroupsComment(row.id)
       }).then(() => {
           this.getData()
    ...
    handleDeleteList(row) {
       this.$confirm('确定删除吗?', '提示', {
           ...
           return delListGroupsComment(this.ids)
       }).then(() => {
           this.getData()
           this.$message.success('删除成功');
       }).catch(() => {
    ...
    handleExport() {
       import('@/vendor/Export2Excel').then(excel => {
           // 表格名称
           const columnNames = ['小组交流ID','小组ID','用户ID','交流内容','交流时间','类型'];
           // 表格属性
           const headerProps = ['id','groupsId','userId','content','creatTime','types'];
           // 表格数据
           allGroupsComment().then(response => {
