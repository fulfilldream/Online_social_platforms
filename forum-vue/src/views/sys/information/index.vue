...
import { dataInformation, dynamicListInformation, listLimitInformation, countInformation, dynamicCountInformation, allInformation, getInformation, detailsGetInformation, dynamicGetInformation, addInformation, editInformation, delInformation, delListInformation } from "@/api/information";
...
export default {
  ...
  methods: {
    ...
    /** 新增按钮操作 */
    handleAdd() {
        this.$router.push({ path: '/sys-information/information/add'})
    },
    /** 编辑按钮操作 */
    handleUpdate(row) {
        this.$router.push({ path: '/sys-information/information/edit/' + row.id })
    },
    /** 详情按钮操作 */
    handleDetails(row) {
        this.$router.push({ path: '/sys-information/information/details/' + row.id })
    },
    // 单个删除按钮操作
    handleDelete(row) {
    ...
    handleDeleteList(row) {
       this.$confirm('确定删除吗?', '提示', {
           ...
           return delListInformation(this.ids)
       }).then(() => {
           this.getData()
           this.$message.success('删除成功');
       }).catch(() => {
    ...
    handleExport() {
       import('@/vendor/Export2Excel').then(excel => {
           // 表格名称
           const columnNames = ['动态ID','标题','内容','摘要','发布者ID','时间','展示图','浏览量'];
           // 表格属性
           const headerProps = ['id','title','content','summary','userId','creatTime','img','readCount'];
           // 表格数据
