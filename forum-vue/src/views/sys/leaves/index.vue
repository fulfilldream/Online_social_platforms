<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tableData" ref="tableRef" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="私信用户" align="center" prop="user.name" min-width="120" :show-overflow-tooltip='true'/>
  ...
  methods: {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            editLeaves(this.form).then(response => {
    ...
    handleExport() {
       import('@/vendor/Export2Excel').then(excel => {
           // 表格名称
           const columnNames = ['私信ID','用户ID','留言内容','留言时间','回复内容','回复时间','老师ID'];
           // 表格属性
           const headerProps = ['id','userId','content','creatTime','replyCon','replyTime','staffId'];
           // 表格数据
           allLeaves().then(response => {
